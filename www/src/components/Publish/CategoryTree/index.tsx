...
function CategoryTree({ register }: CategoryTree) {
  ...
  const treeRender = (node: CategoryTreeItem): ReactNode => {
    //这里不允许 number 否则警告
    return (
      <TreeItem nodeId={node.id + ''} key={node.id} label={node.name}>
  ...
  const treeView = (
    <TreeView
      multiSelect
      onNodeSelect={treeItemHandleChoose}
